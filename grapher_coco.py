...
class GraphOCR:
    ...
    def get_features(self):
        ...
        text_feature = self.get_text_features(all_texts)
    ...
    def get_text_features(self, all_text):
        text_features = np.array([]).reshape(0,9)
        for text in all_text:
            ...
            for char in text:
                ...
                elif char in pythainlp.thai_consonants:n_thai_consonants += 1
                elif char in pythainlp.thai_vowels:n_thai_vowels += 1
                elif char in pythainlp.thai_tonemarks:n_thai_tonemark += 1
                elif char in pythainlp.thai_signs:n_thai_signs += 1
                elif char in pythainlp.thai_digits:n_thai_digits += 1
                else:n_special += 1
            num_array = np.array([[n_alpha, n_numeric, n_special, n_thai_consonants, 
            n_thai_vowels, n_thai_tonemark, n_thai_signs, n_thai_digits, n_thai_signs]])
            text_features = np.append(text_features, num_array/num_array.sum(), axis = 0)
